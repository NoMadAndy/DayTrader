...
- ~~Semantic-Deduplikation via Embeddings~~ ✓ 2026-04-13 (Sprint C6 Teil A): `POST /api/ml/embed/batch` liefert FinBERT-CLS-Embeddings (768-dim, kein extra VRAM); Backend gruppiert via Cosine-Similarity (Default 0.92) und behält pro Cluster das aktuellste Item. Toggle: `ENABLE_SEMANTIC_DEDUP=true` + `SEMANTIC_DEDUP_THRESHOLD=0.92`. IC-Tracking in DB (Teil B) noch offen.
...
- ~~**RL-Legacy-Agents entsorgen oder retrainen**~~ ✓ 2026-04-15: Beide Legacy-Agents (`aggressive_momentum` + `balanced_trader1`) via DELETE /agents/{name} entsorgt + aus PRESET_AGENT_CONFIGS entfernt. DB-Drift-Check: 0 Referenzen. 8 saubere 2407-dim Agents verbleiben.
