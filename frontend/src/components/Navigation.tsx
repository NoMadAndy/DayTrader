...
import { subscribeToAuth, getAuthState, type AuthState } from '../services/authService';
  ...
  return (
    <header className="border-b border-slate-700/50 bg-slate-900/50 backdrop-blur-sm sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-3 py-1.5">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center">
            {authState.isAuthenticated ? (
              <Link
                to="/settings"
                className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-slate-700/50 hover:bg-slate-700 transition-colors"
              ...
              >
                ...
                <span className="hidden sm:inline text-xs text-gray-300">{authState.user?.username}</span>
              </Link>
            ) : (
              <Link
                to="/settings"
                className="flex items-center gap-1.5 px-2 py-1 rounded-md text-gray-400 hover:text-white hover:bg-slate-700/50 transition-colors"
                title={t('nav.login')}
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex items-center">
            {/* Desktop Navigation - Icons with labels for important items */}
            <nav className="hidden md:flex items-center gap-0.5">
              {navItems.map((item, index) => {
                const isActive = location.pathname === item.path;
                // Show labels for first 4 main nav items
                const showLabel = index < 4;
                return (
                  <Link
                    key={item.path}
                    to={item.path}
            ...
            <button
              ...
              className="md:hidden p-1.5 rounded-md text-gray-400 hover:text-white hover:bg-slate-700/50 transition-colors"
              aria-label="Menu"
            >
              {mobileMenuOpen ? (
        ...
        {mobileMenuOpen && (
          <nav className="md:hidden mt-2 pt-2 border-t border-slate-700/50">
